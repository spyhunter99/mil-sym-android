...
public final class flot {
    ...
    protected static int GetAnchorageCountDouble(POINT2[] vbPoints, int numPts) {
        int lTotalpts = 0;
        try {
            //declarations
            int j = 0, lNumSegs = 0;
    ...
    protected static int GetFlot2Double(POINT2[] vbPoints2, int numPts, int lineType) {
        ...
        try {
            ...
            lFlotCount = GetFlotCount2Double(vbPoints2, numPts, lineType);
            if (lFlotCount <= 0) {
                return 0;
            }
            ...
            for (l = 0; l < numPts - 1; l++)
            {
                ...
                if (lNumSegs > 0) {
                    ...
                    numSegPts = GetFlotSegment2(vbPoints, l, points, lineType, bFlip, lDirection, lLastDirection);
                                        ...
                                        if (j < numSegPts - 1) {
                                            ...
                                            if (j < numSegPts - 2)
                                            {
                                                if (lineType == TacticalLines.WFG) {
                                                    ...
                                                    style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 10, 5);
                                                    style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 20, 20);
                                                    style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 30, 0);
                                                    style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 70, 5);
                                                }
                                                if (lineType == TacticalLines.WFY) {
                                                    pt2 = new POINT2(style10Points[style10Counter - 1]);
                                                    style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 10, 5);	//pt before 1st break
                                                    style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 15, 0);	//1st pt after 1st break;
                                                    crossPt1 = lineutility.ExtendDirectedLine(style10Points[style10Counter - 1], pt1, style10Points[style10Counter - 1], 3, 5, 0);
                                                    style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 25, 5);	//2nd point after 1st break;
                                                    crossPt2 = lineutility.ExtendDirectedLine(style10Points[style10Counter - 1], pt1, style10Points[style10Counter - 1], 2, 5, 5);
                                                    style10Points[style10Counter++] = new POINT2(crossPt1);
                                                    style10Points[style10Counter++] = new POINT2(crossPt2);
                                                    style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 30, 0);	//1st pt after 2nd break
                                                    style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 60, 5);	//2nd pt after 2nd break
                                                }
                                            }
                                        } else {
                                            pt2.x = vbPoints[2 * l];
                                            ...
                                            style10Points[style10Counter++] = lineutility.ExtendAlongLineDouble(pt2, pt1, 40, 5);
    ...
    private static int GetFlotSegment2(int[] vbPoints,
            int segment,
            int[] points,
            int lineType,
            ref<int[]> bFlip,
        ...
        try {
            ...
            for (m = 0; m < nNumSegs; m++) {
                ...
                CalcAnglePoints(lLocx, lLocy, dAngle, arcpoints, 10);
    ...
    protected static int GetOccludedPointsDouble(POINT2[] pLinePoints,
            int numPts,
            int lineType)
    {
        int nTotalCounter = 0;
        try
        {
            ...
            double dSpikeSize = 20, dIncrement = 50;
            int[] vbPoints = null;
            ...
            for (j = 0; j < numPts - 1; j++) {
                ...
                if (lNumFlots > 0) {
                    ...
                    GetFlotSegment2(vbPoints, j, points, lineType, bFlip, lDirection, lLastDirection);
                    for (l = 0; l < lNumFlots * 10; l++)
                    {
                        ...
                        if ((nFlotCounter + 1) % 10 == 0) {
                            if (lineType == (long) TacticalLines.OCCLUDED ||
                                ...
                                if (d2 > d1) {
                                    pFlotPoints[nFlotCounter] = lineutility.ExtendLineDouble(pLinePoints[j + 1], pLinePoints[j], -d1 - 20);
                                } else {
                                    pFlotPoints[nFlotCounter] = lineutility.ExtendLineDouble(pLinePoints[j], pLinePoints[j + 1], -d2 + 20);
                                }
                                if (lineType == (long) TacticalLines.OCCLUDED) {
    ...
    private static int GetFlotSegment(int[] vbPoints,
        ...
        try {
            ...
            for (m = 0; m < nNumSegs; m++) {
                lLocx = (int) (vbPoints[2 * segment] + (m + 0.5) * (vbPoints[2 * segment + 2] - vbPoints[2 * segment]) * 20 / dDistance);
                lLocy = (int) (vbPoints[2 * segment + 1] + (m + 0.5) * (vbPoints[2 * segment + 3] - vbPoints[2 * segment + 1]) * 20 / dDistance);
    ...
    protected static int GetFlotDouble(POINT2[] vbPoints2, int numPts) {
        ...
        try {            
            ...
            for (l = 0; l < numPts - 1; l++) {
                ...
                if (lNumSegs > 0) {
                    points = new int[lNumSegs * 30];
                    numSegPts = GetFlotSegment(vbPoints, l, points,bFlip,lDirection,lLastDirection);
                    for (j = 0; j < numSegPts; j++) {
                        x1 = points[k];
                        y1 = points[k + 1];
                        z = points[k + 2];
    ...
    protected static int GetOFYPointsDouble(POINT2[] pLinePoints,
            int numPts,
            int lineType)
    {
        int nTotalCounter = 0;
        try
        {
            ...
            lDirection.value = new int[1];
            lLastDirection.value = new int[1];
            lTotalPoints = GetOFYCountDouble(pLinePoints, numPts, (int) lineType);
            ...
            for (j = 0; j < numPts - 1; j++) {
                ...
                if (lNumFlots > 0) {
                    ...
                    for (l = 0; l < lNumFlots * 10; l++) {
                        ...
                        if ((nFlotCounter + 1) % 10 == 0) {
                            if (lineType == (long) TacticalLines.OFY) {
                ...
                for (l = 0; l < nSpikeEndCounter; l++) {
                    ...
                    tempPoint = lineutility.ExtendDirectedLine(pLinePoints[j], pLinePoints[j + 1], pSegmentPoints[nSegmentCounter - 2], 2, 5, 0);
                    pSegmentPoints[nSegmentCounter++] = new POINT2(tempPoint);
                    tempPoint = lineutility.ExtendDirectedLine(pLinePoints[j], pLinePoints[j + 1], pSegmentPoints[nSegmentCounter - 2], 3, 5, 5);
                    pSegmentPoints[nSegmentCounter++] = new POINT2(tempPoint);
                }
                if (nSpikeCounter == 0) {
                    pSpikePoints[nSpikeCounter] = new POINT2(pLinePoints[j]);
    ...
    protected static int GetSFPointsDouble(POINT2[] pLinePoints,
            int numPts,
            int lineType) {
        int nTotalCounter = 0;
        try {
            ...
            POINT2 pt0 = new POINT2(), tempPoint = new POINT2();
            POINT2[] pFlotPoints = null;
            double dSpikeSize = 20, dIncrement = 80;
            int[] vbPoints = null;
            ...
            for (j = 0; j < numPts - 1; j++) {
                ...
                for (k = 0; k < lNumSpikes - 1; k++) //get the spike
                {
                    ...
                    if (lineType == (long) TacticalLines.SFY) {
                        ...
                        //the cross points
                        pSpikePoints[nSpikeCounter + 4] = lineutility.ExtendDirectedLine(pSpikePoints[nSpikeCounter], pLinePoints[j], pSpikePoints[nSpikeCounter], 2, 5, 25);
                        pSpikePoints[nSpikeCounter + 5] = lineutility.ExtendDirectedLine(pSpikePoints[nSpikeCounter + 3], pLinePoints[j], pSpikePoints[nSpikeCounter + 3], 3, 5, 5);
                        nSpikeCounter += 6;
                        ...
                        //the cross points
                        pSpikePoints[nSpikeCounter + 4] = lineutility.ExtendDirectedLine(pSpikePoints[nSpikeCounter], pLinePoints[j + 1], pSpikePoints[nSpikeCounter], 3, 5, 19);
                        pSpikePoints[nSpikeCounter + 5] = lineutility.ExtendDirectedLine(pSpikePoints[nSpikeCounter + 3], pLinePoints[j + 1], pSpikePoints[nSpikeCounter + 3], 2, 5, 5);
                        nSpikeCounter += 6;
